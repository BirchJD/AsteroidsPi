...
class AstroShip:
   ...
   def Draw(self, ThisSurface):
      for self.Count in range(self.MAX_SHOTS):
         self.Shots[self.Count].Draw(ThisSurface)
...
#/************************************/
               for self.Count in range(0, self.FRAME_POINTS * 2, 2):
                  Common.DrawLine(ThisSurface, self.OldFrame[self.Count].x, self.OldFrame[self.Count].y, self.OldFrame[self.Count+1].x, self.OldFrame[self.Count+1].y, (0x00, 0x00, 0x00))

#  /************************************/
# /* Erase previous position of ship. */
#/************************************/
            Common.DrawLines(ThisSurface, self.OldFrame, self.FRAME_POINTS, (0x00, 0x00, 0x00))

#  /******************************************/
# /* Draw the ship in the current position. */
#/******************************************/
            Common.DrawLines(ThisSurface, self.DisplayFrame, self.FRAME_POINTS, (0x00, 0xFF, 0x00))

#  /******************/
# /* Remove thrust. */
#/******************/
            for self.Count in range(self.THRUST_POINTS):
               Common.DrawLine(ThisSurface, self.ThrustTrail[self.Count].x, self.ThrustTrail[self.Count].y, self.ThrustTrail[self.Count].x + 1, self.ThrustTrail[self.Count].y + 1, (0x00, 0x00, 0x00))
...
#/*****************************************/
            if self.ThrustFlag == True:
               for self.Count in range(self.THRUST_POINTS):
                  self.ThrustTrail[self.Count].x = self.xOffset + random.randrange(9) - 4 + (15 * math.sin(self.FrameStep * self.Angle + self.OneDegree * 180))
                  self.ThrustTrail[self.Count].y = self.yOffset + random.randrange(9) - 4 + (15 * math.cos(self.FrameStep * self.Angle + self.OneDegree * 180))
                  Common.DrawLine(ThisSurface, self.ThrustTrail[self.Count].x, self.ThrustTrail[self.Count].y, self.ThrustTrail[self.Count].x + 1, self.ThrustTrail[self.Count].y + 1, (0x00, 0xFF, 0x00))
...
#/******************************************/
            if self.ExplodeFrame < self.MAX_EXPLODE_FRAME - 1:
               for self.Count in range(0, self.FRAME_POINTS * 2, 2):
                  ...
                  Common.DrawLine(ThisSurface, self.DisplayFrame[self.Count].x, self.DisplayFrame[self.Count].y, self.DisplayFrame[self.Count + 1].x, self.DisplayFrame[self.Count + 1].y, (0x00, 0xFF, 0x00))
...
# /* Erase previous position of ship. */
#/************************************/
            Common.DrawLines(ThisSurface, self.LifeDisplayFrame, self.FRAME_POINTS, (0x00, 0x00, 0x00))

#  /******************************************/
# /* Draw the ship in the current position. */
#/******************************************/
            if self.Lives > self.LifeCount:
               Common.DrawLines(ThisSurface, self.LifeDisplayFrame, self.FRAME_POINTS, (0x00, 0xFF, 0x00))

#  /******************/
# /* Redraw scores. */
#/******************/
      self.PlayerScore.Draw(ThisSurface)
   ...
   def Reset(self):
      if self.Lives == False:
         ...
         self.Fade = 0
         self.Angle = self.SHIP_START_ANGLE
         self.xOffset = (self.xMax + self.SHIP_WIDTH) / 2
         self.yOffset = (self.yMax + self.SHIP_HEIGHT) / 2
         self.xVelocity = 0
         self.yVelocity = 0
         self.Lives = self.MAX_LIVES
